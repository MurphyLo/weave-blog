...
| 来源项目 | 贡献 | 本项目的对应物 |
|---|---|---|
| `../base` | 自定义文本选区系统（DOM Range 布局管线 + SVG 统一路径动画），未来的阅读体验层 | 本阶段**只预留接口**：MDX 渲染层输出 `data-block` / `data-atomic` 标记，详见 [selection-contract.md](./selection-contract.md) |
...
│       └── article.css          # benji agentation.css + MDX prose 扩展
├── next.config.ts               # initOpenNextCloudflareForDev() + images.unoptimized
├── open-next.config.ts          # staticAssetsIncrementalCache（关键！见 §7）
...
rehype:  rehype-slug → rehype-unwrap-images → rehype-pretty-code(shiki)
         → [rehype-katex, 仅当 math] → rehype-data-block（必须最后）
```
...
- `rehype-data-block` 必须最后跑：它需要看到 pretty-code 包装后的 figure、katex 展开后的最终块结构。
...
`CodeFigure` 的复制按钮从渲染后 `<pre>` 的 `textContent` 取文本（不需要 rehype 附加原始代码属性），按钮带 `aria-hidden` + `tabIndex={-1}`（选区契约要求，见 selection-contract.md §3）。
...
1. **接入 base 选区系统**（主要目标）：把 `useTextLayout` / `useTextSelection` / `SelectionLayer` 移植为文章页的阅读层，挂在 `.article` 容器上。内容侧契约已就绪（见 selection-contract.md），预计只需一个 `"use client"` 的 `SelectionProvider` 包装 `{content}`。
2. 行间评论/富组件插入（base CLAUDE.md 路线图）——`data-atomic` + 块分裂机制。
