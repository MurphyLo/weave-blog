...
- 22 个顺序 `data-block`（p / 标题 / li / pre / blockquote>p / 脚注 li 等），零嵌套。
- 5 个 `data-atomic`（Figure 图片、Video、Demo×2（分别内含 Poll、Chat 交互岛）、CTACard），内部无 data-block——交互岛的按钮/文本位于 Demo 的 atomic 子树内，walker 不会递归进去，无需额外 aria-hidden。
